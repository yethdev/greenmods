<script lang="ts">
    ...
    import type { Facet, SortDirection, SortMode } from "@modhost/api";
    ...
    const searchHref = $derived(`${base}${routePath}`);
    const canonicalUrl = $derived(absoluteSiteUrl(canonicalPath));
    const visibleLoaders = $derived(
        ($loaders || []).filter(
            (loader) =>
                allowedLoaderIds.length === 0 || allowedLoaderIds.includes(loader.id.toLowerCase()),
        ),
    );
    const visibleVersions = $derived(
        ($gameVersions || []).filter(
            (version) => allowedVersionIds.length === 0 || allowedVersionIds.includes(version.id),
        ),
    );
    const visibleTags = $derived($tags.filter((tag) => !hiddenTagIds.includes(tag.id)));
    const searchedVersions = $derived(
        visibleVersions.filter((v) => v.id.toLowerCase().includes(versionSearch.toLowerCase())),
    );
    ...
    const searchTitle = $derived(pageTitle);
    const searchDescription = $derived(
        $currentQuery ? `${pageDescription} Query: ${$currentQuery}.` : pageDescription,
    );

    const sanitizeLoaderFilters = (values: string[]) =>
        values.filter(
            (value) =>
                allowedLoaderIds.length === 0 || allowedLoaderIds.includes(value.toLowerCase()),
        );
    const sanitizeVersionFilters = (values: string[]) =>
        values.filter((value) => allowedVersionIds.length === 0 || allowedVersionIds.includes(value));
    const sanitizeTagFilters = (values: string[]) =>
        values.filter((value) => !hiddenTagIds.includes(value));
    ...
    onMount(() => {
        const loadPage = async () => {
            ...
            try {
                const val = queryLoaderFilters ? JSON.parse(queryLoaderFilters) : [];
                loaderFilters = sanitizeLoaderFilters(val);
            } catch {
                loaderFilters = [];
            }

            try {
                const val = queryVersionFilters ? JSON.parse(queryVersionFilters) : [];
                versionFilters = sanitizeVersionFilters(val);
            } catch {
                versionFilters = [];
            }

            try {
                const val = queryTagFilters ? JSON.parse(queryTagFilters) : [];
                tagFilters = sanitizeTagFilters(val);
            } catch {
                tagFilters = [];
            }
    ...
    $effect(() => {
        const facets: Facet[] = [...defaultFacets];
    ...
    const toggleLoaderFilter = (id: string) => {
        return () => {
            ...
            else loaderFilters = [...loaderFilters, id];
    ...
    const toggleVersionFilter = (id: string) => {
        return () => {
            ...
            else versionFilters = [...versionFilters, id];
...
<div class="flex h-full w-full flex-col items-start md:flex-row md:justify-between">
    ...
    >
        ...
        {#if $currentQuery || loaderFilters.length > 0 || versionFilters.length > 0 || tagFilters.length > 0}
            <button
        ...
        >
            <h1 class="mb-2 text-lg md:mb-0">
                ...
                {:else}
                    ...
                    <a href={searchHref} class="anchor no-underline">
                        {$searchResults.total == 1
            ...
            >
                <div class="flex flex-row flex-wrap items-center space-x-1 md:mr-8 md:space-x-2">
                    ...
                    {#if $searchResults.pages > 3}
                        ...
                        >
                    {:else}
                        {#each new Array($searchResults.pages) as _, pageIndex}
                            <button
                                class="variant-glass-primary btn btn-icon-sm hover:variant-ghost-primary text-center font-bold transition-all"
                                class:!variant-filled-primary={currentPage == pageIndex + 1}
                                onclick={() => {
